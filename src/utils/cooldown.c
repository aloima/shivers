...
void setup_cooldown_memory() {
	cooldown_memory = allocate(NULL, 1, sizeof(struct CooldownMemory));
}
...
void add_cooldown(char *user_id) {
	...
	cooldown_memory->cooldowns = allocate(cooldown_memory->cooldowns, cooldown_memory->size, sizeof(struct Cooldown));

	struct Cooldown cooldown;
	cooldown.timestamp = get_timestamp(NULL);
	cooldown.user_id = allocate(NULL, strlen(user_id) + 1, sizeof(char));
...
void remove_cooldown(char *user_id) {
	...
	for (size_t i = at + 1; i < cooldown_memory->size; ++i) {
		cooldown_memory->cooldowns[i - 1].user_id = allocate(NULL, strlen(cooldown_memory->cooldowns[i + 1].user_id) + 1, sizeof(char));
